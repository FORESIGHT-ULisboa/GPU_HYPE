...
def doubleParetoSorting(x0, x1):
    ...
    for i0 in idx[1:]:
        if x0[i0]>=left[-1] and x0[i0]<=right[-1]:
            ...
            #check existing fronts until the point is non-dominated
...
def convexSorting(x0, x1):
    ...
    #plt.savefig(r'C:\Users\ruism\OneDrive\Ambiente de Trabalho\pareto.png', dpi=300)
        ...
        if len(fronts[i0])>0:
            ...
            for i1 in range(i0+1, len(fronts)):
                if len(fronts[i1])>0 and x0[fronts[i0][-1]]<x0[fronts[i1][-1]]:
                    raise(Exception('Something wrong!!! To check'))
