...
from functions import mpld3Correct, processBands
...
class Display(object):
    ...
    '''
    #Nota 1/4/24 Falaste com o Zé sobre usar ou não o mpld3 para fazer gráficos para a tese:
    #Basta mudares usares o matplotlib já aqui programdo e mudares o background
    #Posteriormente para o tethys isto será reintegrado, mas tentamos fazer por fora. Criando uma classe mãe para esta
    #Também não te esqueças que falamos qualquer coisa de um callback para poder cancelar o processo de treino. Para não se perder tempo. 
    ...
    def showPlots(self, data, aggregated, save=None, png=True):
        ...
        #plt.pause(5)
        if save:
            self.save_pickle(fig, save)
            if png:
                png_save = str(save).replace('.pplot', '.png')
                fig.savefig(png_save, dpi=300)
        return aggregated
...
class PlotGPU(object):
    ...
    def timeseries(self, aggregated):
        ...
        if isinstance(self.hTimeSeries, type(None)):
            #===================================================================
            # self.colors = [str(d0) for d0 in np.linspace(0, 1, int(np.floor(self.bands.shape[1]/2)))]
            #===================================================================
    ...
    def to_mpld3(self):
        plotDict = mpld3.fig_to_dict(self.fig)
...
class GPU(object):
        ...
        for i0 in range(epochs):
            ...
            if i0 % self.opt['displayEach']==0 or i0==epochs-1:
                ...
                if  save_fig is not None:
                    save_fig = save_fig.parent / (save_fig.stem + f'_epoch_{i0}.pplot')
                display.prepare(data , save_fig=save_fig)

        # Final operations
            # update the models with the final population
        # if isinstance(self.modelObject, linearModel):
    ...
    def load(cls, fileName):
        ...
        '''
        import sys
        # Handle module restructuring: HYPE -> conceptual.HYPE
    ...
    def __iteration(self):
        ...
        # update frontLevels
        #===================================================================
        # self.frontLevels = jointFrontLevels[toKeepIdxs,]
        #===================================================================
    ...
    def __processVariableBounds(self):
        ...
        #temporary:
        # if isinstance(self.modelObject, linearModel):
        #     self.minBounds = np.zeros((self.opt['population'], self.opt['variables']))
        #     self.maxBounds = np.ones((self.opt['population'], self.opt['variables']))
        #     return None
    ...
    def _penalize_distant_errors(self, fit):
        ...
        #Atention this is not working correctly, is necessary to verify the problem,
        #it could be a problem with the convexSorting function or of the selecting function
        #The problem was a missing + signal, hopefully...

        # if self.opt['forceNonExceedance'] == None:
        #     self.opt['forceNonExceedance'] = np.floor(fit.mean(axis=0)[1])/10
    ...
    def __crowding(self, simulations, fit, frontLvls):
        ...
        # more models on the edges
        #The with statment is used to ignore the warnings of the numpy and
        #allow operations as division by zero occur, being the result infinite
                ...
                for i1 in range(len(tmpBase)-2, 0, -1):
                    if np.isnan(tmp):
                        ...
                        if ~np.isnan(tmpBase[i1]):
                            #This part of the code is used to solve some problem that occurs, in the interpolation, when the values are too close to each other
