...
def mpld3Correct(x):
    ...
    if isinstance(x, dict):
        x = {k: mpld3Correct(v)  for k, v in x.items()}
    elif isinstance(x, (list, tuple)):
        x = [mpld3Correct(i) for i in x]
...
def mpld3RemoveRandomVariables(x):
    
    if isinstance(x, dict):
        x = {(k):(mpld3RemoveRandomVariables(v) if k!='id' and k!='sharex' and k!='sharey' else 0) for k, v in x.items()}
    elif isinstance(x, (list, tuple)):
        x = [mpld3RemoveRandomVariables(i) for i in x]
